...
void Version::GetOverlappingInputs(int level, const InternalKey* begin,
  ...
  // implement get over lapping inputs
  inputs->clear();
  ...
  for (size_t i = 0; i < files_[level].size();) {
    ...
    if (level == 0) {
      ...
      if (end != nullptr && user_cmp->Compare(file_end, user_end) > 0) {
        user_end = file_end;
        overlap = true;
      }
      if (overlap) {
